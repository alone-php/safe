...
trait Mov {
     ...
     * @return mixed
     */
    public static function movDeArr(array $arr): mixed {
     ...
     * @return mixed
     */
    public static function movDesArr(array $arr): mixed {
        return static::arrDecrypt($arr, function($data, $random) {
    ...
    protected static function arrDecrypt(array $arr, callable $callable, array $array = []): mixed {
        try {
            if (!empty($data = static::getArr($arr, 'data')) && !empty($random = static::getArr($arr, 'random'))) {
                if (!empty($json = $callable($data, $random))) {
                    $array = static::isJson($json);
                    return !empty($array) ? $array : $json;
                }
            }
            return $array;
        } catch (Throwable $e) {
