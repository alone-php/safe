...
namespace AlonePack\Phpseclib3\File;
...
use AlonePack\Phpseclib3\Crypt\PublicKeyLoader;
use AlonePack\Phpseclib3\Crypt\Random;
use AlonePack\Phpseclib3\Crypt\RSA;
use AlonePack\Phpseclib3\Crypt\RSA\Formats\Keys\PSS;
use AlonePack\Phpseclib3\Exception\UnsupportedAlgorithmException;
use AlonePack\Phpseclib3\File\ASN1\Element;
use AlonePack\Phpseclib3\File\ASN1\Maps;
...
class X509
{
     ...
     *
     * @see \AlonePack\Phpseclib3\File\X509::getDN()
     */
    const DN_ARRAY = 0;
    /**
     * Return string
     ...
     *
     * @see \AlonePack\Phpseclib3\File\X509::saveX509()
     * @see \AlonePack\Phpseclib3\File\X509::saveCSR()
     * @see \AlonePack\Phpseclib3\File\X509::saveCRL()
     */
    const FORMAT_PEM = 0;
    /**
     * Save as DER
     ...
     *
     * There's no guarantee \AlonePack\Phpseclib3\File\X509 is going to re-encode an X.509 cert in the same way it was originally
    ...
    public function saveX509(array $cert, $format = self::FORMAT_PEM)
    {
        ...
        /* in the case of policyQualifiers/qualifier, the type has to be \AlonePack\Phpseclib3\File\ASN1::TYPE_IA5_STRING.
           \AlonePack\Phpseclib3\File\ASN1::TYPE_PRINTABLE_STRING will cause OpenSSL's X.509 parser to spit out random
           characters.
         */
                    ...
                    case 'id-ce-certificatePolicies':
                        for ($j = 0; $j < count($value); $j++) {
                            ...
                            for ($k = 0; $k < count($value[$j]['policyQualifiers']); $k++) {
                                ...
                                $subvalue = &$value[$j]['policyQualifiers'][$k]['qualifier'];
                                if ($map !== false) {
                                    // by default \AlonePack\Phpseclib3\File\ASN1 will try to render qualifier as a \AlonePack\Phpseclib3\File\ASN1::TYPE_IA5_STRING since it's
                                    // actual type is \AlonePack\Phpseclib3\File\ASN1::TYPE_ANY
    ...
    {
        if (!is_string($extnId)) { // eg. if it's a \AlonePack\Phpseclib3\File\ASN1\Element object
            return true;
        }
     ...
     * Set public key
     *
     * Key needs to be a \AlonePack\Phpseclib3\Crypt\RSA object
     *
     * @param PublicKey $key
     * @return void
     */
     ...
     * Gets the public key
     *
     * Returns a \AlonePack\Phpseclib3\Crypt\RSA object or a false.
     *
     * @return mixed
     */
    ...
    public function sign(X509 $issuer, X509 $subject)
    {
        ...
        if (isset($subject->domains) && count($subject->domains)) {
            $altName = array_map(['\AlonePack\Phpseclib3\File\X509', 'dnsName'], $subject->domains);
        }
        ...
        // save $tbsCertificate in case there are any \AlonePack\Phpseclib3\File\ASN1\Element objects in it
    ...
    public function signCSR()
    {
        ...
        // resync $this->signatureSubject
        // save $certificationRequestInfo in case there are any \AlonePack\Phpseclib3\File\ASN1\Element objects in it
    ...
    public function signSPKAC()
    {
        ...
        // resync $this->signatureSubject
        // save $publicKeyAndChallenge in case there are any \AlonePack\Phpseclib3\File\ASN1\Element objects in it
    ...
    public function signCRL(X509 $issuer, X509 $crl)
    {
        ...
        // resync $this->signatureSubject
        // save $tbsCertList in case there are any \AlonePack\Phpseclib3\File\ASN1\Element objects in it
        $tbsCertList = $this->currentCert['tbsCertList'];
     ...
     * - Key object
     * - \AlonePack\Phpseclib3\File\X509 object with public or private key defined
     * - Certificate or CSR array
     * - \AlonePack\Phpseclib3\File\ASN1\Element object
     * - PEM or DER string
     *
    ...
    public function computeKeyIdentifier($key = null, $method = 1)
    {
        ...
        switch (true) {
            ...
            case $key instanceof X509:
                ...
                return false;
            default: // Should be a key object (i.e.: \AlonePack\Phpseclib3\Crypt\RSA).
